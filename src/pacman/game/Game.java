...
import pacman.QLearning.QStore;
...
public final class Game
{
	...
	public void advanceGameWithQStore(MOVE pacManMove,EnumMap<GHOST,MOVE> ghostMoves)
	{
        advanceGame(pacManMove, ghostMoves);
        Node newState = currentMaze.graph[getPacmanCurrentNodeIndex()];

        // Previous move related information
        MOVE previousMove = pacman.lastMoveMade;
        int previousNodeIndex = newState.neighbourhood.get(previousMove.opposite());
        Node previousState = currentMaze.graph[previousNodeIndex];
        QStore qstore = new QStore(this);
        Double previousQValue = qstore.getQValue(previousState, previousMove);


        // Reward Strategy
        // if pacman was Eaten then we get negative reward
        // if pillWasEaten then we get a high positive rewards
        // and for everything else whether ghostWasEaten, powerpillWasEaten etc. we keep very
        ...
        // we check that if by any means have we reduced the distance between
        // pacMan and a non edible ghost
        for(GHOST g: GHOST.values()) {
            if(!isGhostEdible(g) && getShortestPathDistance(
                    newState.nodeIndex,
                    getGhostCurrentNodeIndex(g)) < getShortestPathDistance(
                            previousNodeIndex, getGhostCurrentNodeIndex(g))) {
                badMove = true;
                break;
            }
        }
        ...
        // Get state-action pair with max Q value from the new state
        Map.Entry<String, Double> qValuetuple = qstore.getMaxQValue(newState);

        // store the Q value for the new state-action pair, here action is chosen
        // such that the Q value that we get is the max Q value for the state,action pair
        // from the Q store
        Double qValueForNewState = qValuetuple.getValue();

        // update the Q value as per the formulae
        // Q(s, a) = (1-Q_LEARNING_RATE)Q(s, a) + Q_LEARNING_RATE*(reward + Q_DISCOUNT_RATE * maxQ(s', a')
        // Where
        // Q(s, a) is the Q value from the previous run for state s and action a
        // Q(s',a') is the Q value of the new state s' and action a'
        // maxQ(s', a') selects a' for s' such that Q value is max from the Q store
        Double newQValue = (1 - Q_LEARNING_RATE) * previousQValue + Q_LEARNING_RATE * (reward +
                Q_DISCOUNT_RATE * qValueForNewState);


        System.out.println("previous state: " + previousState.nodeIndex);
        System.out.println("previous move: " + previousMove);
        System.out.println("previous Q value for state: " + previousQValue);
        System.out.println("new State, Action value " + qValuetuple.toString());
        System.out.println("Final Q value for state " + previousNodeIndex + " : "+  newQValue);

        // Update (state-action pair from previous run that lead to the current state and reward)
        qstore.setQValue(previousState, previousMove, newQValue);
